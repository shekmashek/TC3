  ...
  const filteredBudgetEntries = useMemo(() => {
    ...
    if (quickFilter !== 'all') {
        if (quickFilter === 'provisions') {
            ...
            entries = entries.filter(e => e.category.startsWith(borrowingCat));
        } else if (quickFilter === 'lendings') {
            const lendingCat = "Prêts (Remboursement de)";
            entries = entries.filter(e => e.category.startsWith(lendingCat));
            ...
            entries = entries.filter(e => epargneSubCategories.includes(e.category));
        }
    }
    return entries;
